...
\begin{itemize}
  ...
  In case of the orthogonal design, we chosen ASK source constellations shifted by $\pi/2$ to 
  form the QAM constellation in the received node. The hierarchical design depends on a particular
  network function (exclusive OR in our case). Prior explaining the composition of the 
  hierarchical constellation, we clarify our goal, that is the received constellation with maximal
  minimum distance between the \emph{network coded data}. To provide a simple example, consider
  two BPSK constellations $s\in\{-1,1\}$. 
  ...
  placed at higher power level compared to the superposed stream. Please see Figures 3,4 in the
  manuscript for demonstration.
 
  Regarding to the generalization of the principle to general network, one can of cause consider
  ...
  network stands for a white space for further research. One should also keep in mind that the
  severity of the synchronization and pre-rotation issues would dramatically grow with the network
  size.
