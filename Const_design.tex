...
\subsection{Three-Step Reference Scenario}
For a more advanced comparison of the proposed method, we consider two conventional \emph{3-step}
communication scenarios. 

The first scenario is equivalent to a conventional routing, where the sources subsequently transmit
...
In the second scenario both sources again transmit their information one by one, but the relay is
...
information at both sources (see Figure \ref{fig:CTUpp_ref3stepNC}).
...
We compared throughput of the proposed adaptive constellation design from Section
\ref{sub:Maximal-Throughput-of} on the USRP devices with optimized tuple ($r_b, r_s, N_b, N_s$) for
each measured point (HW-pre-rotation on) with the reference scenarios in Figures \ref{fig:CTUpp_comparison-schemaa} and
\ref{fig:CTUpp_comparison-schemab}. We also considered the proposed method with
pre-rotation turned off as in Section \ref{sub:Robustness_Analysis} for comparison. Finally, we
added two theoretical limits for the 3-step scenarios, where ideal Gaussian alphabets are assumed.
