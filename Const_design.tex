...
The second scenario is equivalent to a conventional routing, where the sources subsequently transmit
...
\begin{itemize}
  ...
  with based on the optimized tuple ($r_b, r_s, N_b, N_s$) for each measured point
  (HW-pre-rotation on).
...
$\gamma_{\mathrm{HSI}}\approx\gamma_{\mathrm{MAC}}$+3dB. The numbers in boxes correspond with a
...
The numbers in boxes correspond with a
particular values (first row:$(N_b, r_b)$, second row:$(N_s, r_s)$)
achieving a given maximal throughput.
