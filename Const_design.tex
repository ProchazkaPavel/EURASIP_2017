...
\item We compare the proposed design with state of the art reference scherios to highlight its
...
\begin{figure*}
\centering{}
\includegraphics[width=0.45\textwidth]{fig/Adaptive_Maps_overall_BC_7\lyxdot 5}~~~
\includegraphics[width=0.45\textwidth]{fig/Adaptive_Maps_overall_BC_15\lyxdot 0}
...
\begin{figure*}
\centering{}
\includegraphics[width=0.45\textwidth]{fig/XOR_map_Througput_coding_Gain7\lyxdot 5}~~~
\includegraphics[width=0.45\textwidth]{fig/XOR_map_Througput_coding_Gain15\lyxdot 0}
...
The first scenario both sources again transmit their information one by one, but the relay is
...
information at both sources, that is the traditional network coding approach (see Figure
\ref{fig:CTUpp_ref3stepNC}).

In the scenario is equivalent to a conventional routing, where the sources subsequently transmit
their data packet towards the relay. Relay then sends a complete source information (no NC coding is
...
To provide a fair comparison, in both reference scenarios we assume that the sources transmit at
double power, as they transmit time is essentially halved when compared with the 2-step scenarios.
...
The theoretical limits for the considered 3-step scenarios can be easily derived (see
...
\end{eqnarray}
where $\mathrm{CA}(\gamma)=\log_2(1+\gamma)$ denotes conventional point to point channel capacity for Gaussian
channel \cite{Cover-Thomas_ElemInfTheory}.
...
Within this experiment, we evaluated on our USRP platform the following situations:
\begin{itemize}
  \item The proposed adaptive constellation design from Section \ref{sub:Maximal-Throughput-of} 
  with optimized tuple ($r_b, r_s, N_b, N_s$) for each measured point
  (HW-pre-rotation on).
  \item The proposed adaptive constellation design with violated assumptions (pre-rotation off)
...
\begin{itemize}
  ...
  theoretical limits even if the pre-rotation is off! The bottleneck for the 3-step routing is BC
  channel, because it needs to pass twice amount of data from relay to destinations compared to
  all other schemes. The 3-step NC approach does not suffer from this and therefore its performance
  is a bit better, nevertheless we pay for orthogonality in the MA stage. The proposed performance
  ...
  in Figure \ref{fig:CTUpp_comparison-schemab}. The 3-step routing works much better compared to the previous
  case, because of the strong BC channel. The conventional 3-step network coding suffers from weak
  HSI channel. The proposed design again significantly outperforms both reference 3-step scenarios.
  We should stress, that the performance gain is also caused by a \emph{partial usage} of the weak
  HSI channel.
\end{itemize}
...
$\gamma_{\mathrm{HSI}}\approx\gamma_{\mathrm{MAC}}$+3dB
